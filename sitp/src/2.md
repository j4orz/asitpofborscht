...
<small>*Plato's Cave (Jan Pietersz Saenredam after Cornelis Cornelisz van Haarlem 1604)*</small><br>
...
Welcome young hacker! In part one of [The Structure and Intepretation of Tensor Programs](./front.md) you have developed a solid foundation in the
mathematical preliminaries and statistical models used throughout the machine learning approach to artificial intelligence.
It's amazing how close you are to of deep learning without you even knowing.
By the end of [Part II. Neural Networks](), we will be one step closer in achieving our quest of building our own **[ChatGPT](https://chatgpt.com/)**
by reproducing GPT2<span class="sidenote-number"></span><span class="sidenote">*Presented in [Language Models are Unsupervised Multitask Learners (Raford et al. 2019)](https://cdn.openai.com/better-language-models/language_models_are_unsupervised_multitask_learners.pdf)*</span>, following Andrej Karpathy's [nanogpt]().
But before we get there, there is some more work for us to do.

</div>

So in [Chapter 4. Learning *Sequences* via Deep Neural Networks with `teenygrad`](),
you will increase the expressivity of the linear models implemented in [Part I](./1.md) by non-linearities to get a class of models known as deep neural networks.
...
following the nets in Andrej Karpathy's [makemore](https://github.com/karpathy/makemore),
...
Then, in [Chapter 5. Accelerating *Sequence Models* on `GPU` in `teenygrad`](#5-accelerating-sequence-models-on-gpu-in-teenygrad-with-cuda-rust),
you will evolve [teenygrad]() from a numerical linear algebra library implemented in [Part I]() to a full blown batteries-included deep learning framework like [**PyTorch**](https://pytorch.org/).
...
After completing part two, you will be ready for [Part III. Scaling Networks](./3.md) of the book where we finally achieve our quest of building our own **[ChatGPT](https://chatgpt.com/)**.
Part three follows the *2020-2025* time period of what is colloquially as the *age of scaling*
where researchers focused on scaling up the generality of generative pretrained transformers
by adding *assistant*-like behavior in a midtraining phase with reinforcement learning with
...
and by adding *reasoning*-like behavior in a posttraining phase with
foobarbaz<span class="sidenote-number"></span><span class="sidenote">*Originally presented in [Introducing OpenAI o1 (OpenAI 2024)](https://openai.com/o1/), and reproduced by open source in [DeepSeek-R1: Incentivizing Reasoning Capability in LLMs via Reinforcement Learning](https://arxiv.org/pdf/2501.12948)*</span>
<!-- We will build off from what we know by [nanogpt]() and build [nanochat]
and evolve `teenygrad`'s implementation for the third and final time in order to build the languages and runtimes necessary for scaling language language models. -->
...
- [3. Self-Supervised Sequence Learning with Deep Networks](#3-self-supervised-sequence-learning-with-deep-networks)
  - [3.1 Optimization with Gradient Descent](#31-optimization-with-gradient-descent)
    - [3.1.1 Foo]()
    - [3.1.2 Bar]()
  - [3.2 Learning Sequences with FNNs](#32-learning-sequences-with-ffns)
    - [3.2.1 Architecture with Lots of Linear Regression](#321-architecture-with-lots-of-linear-regression)
    - [3.2.2 Loss Function with Cross Entropy](#322-loss-function-with-cross-entropy)
    - [3.2.3 Optimization with (Stochastic) Gradient Descent](#323-optimization-with-stochastic-gradient-descent)
  - [3.3 Learning Sequences with CNNs]()
  - [3.4 Learning Sequences with RNNs]()
  - [3.5 Learning Sequences with GPTs]()
  - [3.6 Double Descent]()
...
### 3.2 Learning Sequences with FFNs
...
- [3.2 Learning Sequences with FFNs](#32-learning-sequences-with-ffns)
...
<small>[$\hookleftarrow$ Table of Contents (§3.2 Learning Sequences with FFNs)](#12-next-token-prediction-is-classification-and-compression)</small>
