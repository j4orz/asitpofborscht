...
<!-- Historically speaking, the progress of artificial intelligence was significantly advanced by the methods of deep learning, which started bearing fruit throughout the 2010s
with magical technology such as <span class="smallcaps">Translate</span>, <span class="smallcaps">Siri</span>, and <span class="smallcaps">Autopilot</span>,
but most recently with <span class="smallcaps">ChatGPT</span> and <span class="smallcaps">Claude</span>. In this second part of the <span class="smallcaps">SITP</span> epic, we will re-produce <span class="smallcaps">GPT-2</span> by building our own <span class="smallcaps">nanochat</span> from scratch. -->

<div class="dropcap">

Welcome young hacker! Glad to see you here. The whimsical whirlwind tour known as
**The Structure and Interpretation of Tensor Programs** (<span class="smallcaps">SITP</span>) started with [Part I. Elements of Networks](./1.md)
where we developed a solid breadth of foundation in the computational, mathematical, and performance-oriented perspectives of the statistical approach to machine learning.
By now, although we've "only" trained what amounts to a single layer network, we are now more than comfortable in the stochastic and infinitely continuous language of software 2.0. That is, although using the precise tools of mathematics to specify architectures, loss functions, and optimizers, the *essence* of the discipline is more akin to synthetic chemistry or microbiology which *grow* chemicals and cells.
In [Part II. Neural Networks](./2.md), we simply continue where we left off.

</div>

**representation learning**<span class="defnote">**representation learning**</span>
**universal approximators**<span class="defnote">**universal approximators**</span>
**double descent**<span class="defnote">**double descent**</span>

Namely, in [§3.1.2 From Localist to Distributional Representations](#312-from-localist-to-distributional-representations),
we cover XYZ and in [§3.1.3 From Linear Perceptrons to Non-Linear Multi Layer Perceptrons](#313-from-linear-perceptrons-to-non-linear-multi-layer-perceptrons).
we increase the expressivity of model architecture from linear perceptrons otherwise known as single-layer networks to non-linear **multi-layer perceptrons**<span class="defnote">**multi-layer perceptron**</span> or **deep neural networks**<span class="defnote">**deep neural network**</span>.
Moreover, because deep networks $\hat{f}_{\theta}$ are no longer linear, in [§3.1.4 From Direct to Iterative Optimization](#314-from-direct-to-iterative-optimization) we transition from from direct optimizatio methods that solve for the exact solution to iterative ones that solve for approximate solutions.

After [Part II. Neural Networks](./2.md), we will be ready for [Part III. Scaling Networks](./3.md) where we finally complete our quest in building our own
language model such as <span class="smallcaps">ChatGPT</span> by following <span class="smallcaps">Nanochat</span> and subsequently, our own deep learning framework capable of running said language model like <span class="smallcaps">PyTorch 2</span>,
by implementing <img class="icon" src="./assets/borscht.png" alt="" style="aspect-ratio: 736 / 510;">[`borscht.compile()`]() and <img class="icon" src="./assets/borscht.png" alt="" style="aspect-ratio: 736 / 510;">[`borscht.distributed()`]().
...
by adding additional phases after **pre-training**<span class="defnote">**pre-training**</span> stochastic parrots.
That is, the **mid-training**<span class="defnote">**mid-training**</span>
...
- [3. Self-Supervised Sequence Learning with Deep Networks](#3-self-supervised-sequence-learning-with-deep-networks)
  - [3.1 Learning Representations with <span class="smallcaps">FFNs</span>](#31-learning-representations-with-ffns)
    - [3.1.1 From Feature Engineering to Representation Learning](#311-from-feature-engineering-to-representation-learning)
    - [3.1.2 From Linear Perceptrons to Non-Linear Multi Layer Perceptrons](#312-from-localist-to-distributional-representations)
    - [3.1.3 From Direct to Iterative Optimization](#313-from-linear-perceptrons-to-non-linear-multi-layer-perceptrons)
  - [3.2 Iterative Optimization](#32-iterative-optimization)
    - [3.2.1 Taylor Series and their Derivatives, Gradients, and Hessians](#321-taylor-series-and-their-derivatives-gradients-and-hessians)
    - [3.2.2 First Order Optimization]() <!-- steepest descent, gradient descent -->
    - [3.2.3 Second Order Optimization]() <!-- Newton's Method, gauss newton method -->
    - [3.2.4 Momentum]()
    - [3.2.5 Weight Decay]()
  - [3.3 Learning Representations with <span class="smallcaps">CNNs</span>]()
  - [3.4 Learning Representations with <span class="smallcaps">RNNs</span>]()
  - [3.5 Learning Representations with <span class="smallcaps">GPTs</span>](#35-learning-representations-with-gpts)
    ...
    - [3.6 Double Descent]()
- [Intermezzo Three: The Language of Neural Networks](#intermezzo-three-the-language-of-neural-networks)
  - [I3.1 Inner Product Spaces, Normed Spaces, Hilbert Spaces](#i31-inner-product-spaces-normed-spaces-hilbert-spaces)
  - [I3.2 Sequences, Continuity, Limits, Derivatives]()
  - [I3.3 Superposition]()
- [4. The Engine of Automatic Differentiation](#4-the-engine-of-automatic-differentiation)
  - [4.1 Gradient-Based Optimization with Automatic Differentiation](#42-gradient-based-optimization-with-automatic-differentation)
    - [4.1.1 Forward Mode with <img class="icon" src="./assets/borscht.png" alt="" style="aspect-ratio: 736 / 510;">`borscht.forward()`]()
    - [4.1.2 Backward Mode with <img class="icon" src="./assets/borscht.png" alt="" style="aspect-ratio: 736 / 510;">`borscht.backward()`]()
    - [4.1.3 Stochastic Gradient Descent with <img class="icon" src="./assets/borscht.png" alt="" style="aspect-ratio: 736 / 510;">`borscht.optim.sgd()`]()
    - [4.1.4 Adam with <img class="icon" src="./assets/borscht.png" alt="" style="aspect-ratio: 736 / 510;">`borscht.optim.adamw()`]()
  - [4.2 Network Primitives](#41-network-primitives)
    - [4.2.1 Representing Models with `Graph<`<img class="icon" src="./assets/borscht.png" alt="" style="aspect-ratio: 736 / 510;">`borscht.Tensor>`]()
    - [4.2.2 Implementing Forward Pass for <img class="icon" src="./assets/borscht.png" alt="" style="aspect-ratio: 736 / 510;">`borscht.nn.Embedding()`]()
    - [4.2.3 Implementing Forward Pass for <img class="icon" src="./assets/borscht.png" alt="" style="aspect-ratio: 736 / 510;">`borscht.nn.Linear()`]()
    - [4.2.4 Implementing Forward Pass for <img class="icon" src="./assets/borscht.png" alt="" style="aspect-ratio: 736 / 510;">`borscht.nn.Tanh()`]()
  - [4.3 From Sequential Machines to Parallel Machines](#41-from-sequential-machines-to-parallel-machines)
  - [4.4 Accelerating Matrix Multiplication on <span class="smallcaps">GPUs</span>]() <!-- simd to simt -->
    - [4.3.1 From Multi Core <span class="smallcaps">CPUs</span> to Many Core <span class="smallcaps">GPUs</span>]()
...
> In which we transition from machine learning to deep learning, which focuses on learning distributional representations automatically with deep nets; introduce such deep nets as multi-layer perceptrons, the non-linear extensions of linear models with the <span class="mod">ReLU</span> and <span class="mod">Embed</span> primitives; transition our optimization techniques to handle non-linear functions with iterative descent methods.
...
#### 3.1.1 From Feature Engineering to Representation Learning
<small>[$\hookleftarrow$ Table of Contents (§3.1 Learning Representations with FFNs)](#31-learning-representations-with-ffns)</small>

<div class="dropcap">

Machine learning of software 2.0 serves as the foundation for training the deep neural networks that power modern-day large language models.
Namely, the discipline models language stochastically as [*next-token prediction*](./1.md#next-token-prediction). Such a distribution is then used to construct a [*classifier*](./1.md#classification) $\hat{f}: \mathscr{X} \to \mathscr{Y}$ that assigns an output token $y$ for a given input sentence $(x^{(1)}, \dots, x^{(t)})$ where $\mathscr{X} := V^{*}$ and $\mathscr{Y} := V$ from [*self-supervised*](./1.md#self-supervised-learning) data $d:=\{(\mathbf{x}^{(i)}, y^{(i)})\}_{i=1}^{n}$.
Regardless of whether $\hat{f}$ is simply a [*discriminative function*](./1.md#discriminant-functions), or estimates the posterior $p(y|x^{1}, \dots, x^{t})$ as a [*discriminative probabilistic model*](./1.md#discriminative-probabilistic-model), or [*generative probabilistic model*](./1.md#generative-probabilistic-model) with a prior and likelihood, it's modeled [*parametrically*](./1.md#parameterized) as $\hat{f}_{\theta}$,
and so the problem of finding a good model reduces to finding a good set of parameters $\theta$ with *parameter estimation*.
...
In [Part I. Elements of Networks](./1.md), after estimating the conditional probability distribution $p(y|x^{1}, \dots, x^{t})$ with the non-parametric n-gram model, we explored model architectures that corresponded to single-layer neural networks where

$$
\begin{aligned}
\hat{f}_{\theta}&: \R^{d} \to \R \\
\hat{f}_{\theta}(x)&:= w^{\top}\phi(x) \\
\\
\ell&: \R^{d} \to \R \\
\ell(\theta) &:= p\log q
\end{aligned}
...
$$
\mathscr{L}(\theta) := p\log q \circ w^{\top}\phi(x) \\
$$

where $\hat{f}_{\theta}$ is then composed as $I^{-1} \hat{f}_{\theta} \circ I$.

**Deep learning**<span class="defnote" data-refine="deep-learning">**deep learning**</span> builds off from such foundation,
namely, programming of software 2.0 with the three step process of selecting a model architecture, scoring it with a loss, and optimizing such loss. However, deep learning focuses on the class of models $\hat{f}_{\theta}$ known as deep neural networks or multi-layer perceptrons that introduce many layers of non-linearities that make them well-suited for **learning representations**<span class="defnote">**representation learning**</span><span class="sidenote-number"></span><span class="sidenote">*See [(Bengio et al., 2012)](https://arxiv.org/pdf/1206.5538) and [(LeCun et al., 2015)](https://www.nature.com/articles/nature14539)*</span>.

**localist representation**<span class="defnote">**localist representation**</span>
**distributional representation**<span class="defnote">**distributional representation**</span>

Localist representations such as one hot vectors are used to program the software 2.0 of machine learning,
whereas **distributional representations**<span class="defnote">**distributional representation**</span> such as **embeddings**<span class="defnote">**embeddings**</span> are used to program the software 2.0 of deep learning. In this chapter, we will learn about the neural network primitive of <span class="mod">Embed</span>.


<span class="lecnote">*<span class="fa-svg"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--! Font Awesome Free 6.2.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free --><path d="M512 256c0 141.4-114.6 256-256 256S0 397.4 0 256S114.6 0 256 0S512 114.6 512 256zM188.3 147.1c-7.6 4.2-12.3 12.3-12.3 20.9V344c0 8.7 4.7 16.7 12.3 20.9s16.8 4.1 24.3-.5l144-88c7.1-4.4 11.5-12.1 11.5-20.5s-4.4-16.1-11.5-20.5l-144-88c-7.4-4.5-16.7-4.7-24.3-.5z"/></svg></span> [Stanford CS224N: NLP with Deep Learning | Lecture 1 - Intro and Word Vectors](https://www.youtube.com/watch?v=DzpHeXVSC5I&list=PLoROMvodv4rOaMFbaqxPDoLWjDaRAdP9D&index=1)*</span>

<span class="lecnote">*<span class="fa-svg"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--! Font Awesome Free 6.2.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free --><path d="M512 256c0 141.4-114.6 256-256 256S0 397.4 0 256S114.6 0 256 0S512 114.6 512 256zM188.3 147.1c-7.6 4.2-12.3 12.3-12.3 20.9V344c0 8.7 4.7 16.7 12.3 20.9s16.8 4.1 24.3-.5l144-88c7.1-4.4 11.5-12.1 11.5-20.5s-4.4-16.1-11.5-20.5l-144-88c-7.4-4.5-16.7-4.7-24.3-.5z"/></svg></span> [Stanford CS224N: NLP with Deep Learning | Lecture 2 - Word Vectors and Language Models](https://youtu.be/DzpHeXVSC5I?list=PLoROMvodv4rOaMFbaqxPDoLWjDaRAdP9D&t=1702)*</span>

<div class="full-bleed">
<iframe height="500px" loading="lazy" src="https://www.youtube.com/embed/wjZofJX0v4M?si=6GPGcxx1VQo6VVY0&amp;start=443" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
</div>
...
| 2.0 goal | 2.0 method ([Part I](./1.md)) | 2.0 method ([Part II](./2.md)) |
|---|---|---|
| evaluation | next-token prediction with stochastics of probability theory and compression of information theory | next-token prediction with stochastics of probability theory and compression of information theory |
| architecture | linear single-layer perceptron with dimensionality of linear algebra | non-linear multi-layer perceptron with dimensionality of linear algebra |
| optimizers | direct solvers with linear systems of linear algebra | iterative solvers with approximation of differential calculus |
systems | speed of light acceleration with rooflines of cpus | speed of light acceleration with rooflines of gpus |

In [§3. Self-Supervised Sequence Learning with Deep Networks](#3-self-supervised-sequence-learning-with-deep-networks),
we will implement different neural network architectures that all express various **inductive biases**<span class="defnote">**inductive biases**</span>. Such inductive biases were all explored during the *2012-2019* time period of what is coloquially known as the **age of research**<span class="defnote">**age of research**</span>. Keeping the goal of next-token prediction constant, we start from the standard **feedforward neural networks**<span class="defnote">**FFN**</span> (<span class="smallcaps">FFN</span>),
and then transition to more novel architectures such as **convolutional neural networks**<span class="defnote">**CNN**</span> (<span class="smallcaps">CNN</span>),
**recurrent neural networks**<span class="defnote">**RNN**</span> (<span class="smallcaps">RNN</span>), and finally, **generative pre-trained transformers**<span class="defnote">**GPT**</span> (<span class="smallcaps">GPT</span>),
specifically by reproducing <span class="smallcaps">GPT-2</span><span class="sidenote-number"></span><span class="sidenote">*Presented in [Language Models are Unsupervised Multitask Learners (Raford et al. 2019)](https://cdn.openai.com/better-language-models/language_models_are_unsupervised_multitask_learners.pdf)*</span> following <span class="smallcaps">Nanogpt</span>.
Because deep neural networks are no longer linear functions of their parameters,
we will need to transition from direct methods to iterative ones with descent-based searches.
Starting from **gradient descent**<span class="defnote">**gradient descent**</span>,
**momentum**<span class="defnote">**momentum**</span>,
**weight decay**<span class="defnote">**weight decay**</span>,
**AdamW**<span class="defnote">**AdamW**</span>.

Then, in [§4. The Engine of Automatic Differentiation](#4-the-engine-of-automatic-differentiation),
you will evolve the implementation of <img class="icon" src="./assets/borscht.png" alt="" style="aspect-ratio: 736 / 510;">[`borscht`](https://github.com/j4orz/borscht) from a numerical linear algebra library like <span class="smallcaps">Numpy</span>
from [§2. The Multidimensional Parallelized Tensor](./1.md#2-the-massively-parallel-multidimensional-tensor) into
a full blown batteries-included deep learning framework like <span class="smallcaps">TORCH</span> which will be able to run the training and inference of the said nets just trained in [§3]().
This means implementing the optimizers for neural network primitives whose evaluations are accelerated with massively parallel processors known as **graphics processing units**<span class="defnote">**GPU**</span> (<span class="smallcaps">GPUs</span>)
...
Consult last few chapters of
[(Jurafsky and Martin 2026)]()<span class="sidenote-number"></span><span class="sidenote">*If Jurafsky and Martin is not your cup of tea, take a look at [(Eisenstein 2018)]().*</span>,
and [(Bishop 2025)](https://www.microsoft.com/en-us/research/people/cmbishop/prml-book/)<span class="sidenote-number"></span><span class="sidenote">*Similarly, you can also take a look at [(Goodfellow et al., 2017)]()*</span> for deep learning.
...
<iframe height="500px" loading="lazy" src="https://projector.tensorflow.org/"  title="Tensorflow Embedding Projector" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>

</div>

**junk**

Newton's classical mechanics must be understood before Einsten's general relativity.
Similarly, so too of Rosenblatt's machine learning before that of Bengio, Hinton, and LeCun's deep learning.
First, we understand that language models such as <span class="smallcaps">Nanogpt</span>, <span class="smallcaps">ChatGpt</span>, and <span class="smallcaps">Claude</span>
...
(expand with coterell formalization)
...
in [§1.4 Parameterizing Classification with Logistic Regression](./1.md#13-parameterizing-classification-with-linear-regression). In either case, both models $f$ are specified *dimensionally*, and, in the case of linear models,
can be implemented with the direct optimization of linear algebra's linear systems as well
...
Linear models such as a single layer of perceptrons are used to program the software 2.0 of machine learning,
whereas **non-linear models**<span class="defnote">**non-linear models**</span> such as **multi-layer perceptrons**<span class="defnote">**multi-layer perceptrons**</span> are used to program the software 2.0 of deep learning. We will compose the primitives of <span class="mod">Embed</span>, <span class="mod">Linear</span>, and <span class="mod">RelU</span> together to form a deep neural network.

</div>

- wed sep.23 after the 3.1 source of flow
- work on architecture 3.1.3 (merge the various representations) and optimizers 3.1.4
- including synthesis from intermezzo
...
todo:glm,exp.
The key entry point into the function class of **deep neural networks**<span class="defnote">**deep neural network**</span><span class="lecnote">*<span class="fa-svg"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--! Font Awesome Free 6.2.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free --><path d="M512 256c0 141.4-114.6 256-256 256S0 397.4 0 256S114.6 0 256 0S512 114.6 512 256zM188.3 147.1c-7.6 4.2-12.3 12.3-12.3 20.9V344c0 8.7 4.7 16.7 12.3 20.9s16.8 4.1 24.3-.5l144-88c7.1-4.4 11.5-12.1 11.5-20.5s-4.4-16.1-11.5-20.5l-144-88c-7.4-4.5-16.7-4.7-24.3-.5z"/></svg></span> [Stanford CS109 | Deep Learning | Lecture 25](https://www.youtube.com/watch?v=MSfI6TTgyl4&list=PLoROMvodv4rOpr_A7B9SriE_iZmkanvUg&index=26)*</span>

<span class="lecnote">*<span class="fa-svg"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--! Font Awesome Free 6.2.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free --><path d="M512 256c0 141.4-114.6 256-256 256S0 397.4 0 256S114.6 0 256 0S512 114.6 512 256zM188.3 147.1c-7.6 4.2-12.3 12.3-12.3 20.9V344c0 8.7 4.7 16.7 12.3 20.9s16.8 4.1 24.3-.5l144-88c7.1-4.4 11.5-12.1 11.5-20.5s-4.4-16.1-11.5-20.5l-144-88c-7.4-4.5-16.7-4.7-24.3-.5z"/></svg></span> [Stanford CS229 Machine Learning | Lecture 7: Neural Networks (Architecture)](https://www.youtube.com/watch?v=fRM41w9jzQo&list=PLaqpC4kq8Gpw&index=7)*</span>


<span class="lecnote">*<span class="fa-svg"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--! Font Awesome Free 6.2.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free --><path d="M512 256c0 141.4-114.6 256-256 256S0 397.4 0 256S114.6 0 256 0S512 114.6 512 256zM188.3 147.1c-7.6 4.2-12.3 12.3-12.3 20.9V344c0 8.7 4.7 16.7 12.3 20.9s16.8 4.1 24.3-.5l144-88c7.1-4.4 11.5-12.1 11.5-20.5s-4.4-16.1-11.5-20.5l-144-88c-7.4-4.5-16.7-4.7-24.3-.5z"/></svg></span> [Stanford CS229 Machine Learning | Lecture 13: LLMs, Next-Word Prediction Loss](https://www.youtube.com/watch?v=lNTajqxxOn4&list=PLaqpC4kq8Gpw&index=14)*</span>
...
<div class="full-bleed">
<iframe height="500px" loading="lazy" src="https://www.youtube.com/embed/-at7SLoVK_I?si=Nss5IlHjEMIgVo92" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
</div>
...
<div class="full-bleed">
<iframe height="500px" loading="lazy" src="https://www.youtube.com/embed/TCH_1BHY58I?si=7O2y7-ctFTm1Mf-r" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
</div>
...
Direct optimization such as solving linear systems of equations with least squares
is used to program the software 2.0 of machine learning,
whereas **iterative optimization**<span class="defnote">**iterative optimization**</span> such as **gradient descent**<span class="defnote">**gradient descent**</span> via **backpropagation**<span class="defnote">**backpropagation**</span><span class="lecnote">*<span class="fa-svg"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--! Font Awesome Free 6.2.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free --><path d="M512 256c0 141.4-114.6 256-256 256S0 397.4 0 256S114.6 0 256 0S512 114.6 512 256zM188.3 147.1c-7.6 4.2-12.3 12.3-12.3 20.9V344c0 8.7 4.7 16.7 12.3 20.9s16.8 4.1 24.3-.5l144-88c7.1-4.4 11.5-12.1 11.5-20.5s-4.4-16.1-11.5-20.5l-144-88c-7.4-4.5-16.7-4.7-24.3-.5z"/></svg></span> [Stanford CS229 Machine Learning | Lecture 8: Neural Networks 2 (Backprop)](https://www.youtube.com/watch?v=ne2ngVAoMG8&list=PLaqpC4kq8Gpw&index=8)*</span><span class="lecnote">*<span class="fa-svg"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--! Font Awesome Free 6.2.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free --><path d="M512 256c0 141.4-114.6 256-256 256S0 397.4 0 256S114.6 0 256 0S512 114.6 512 256zM188.3 147.1c-7.6 4.2-12.3 12.3-12.3 20.9V344c0 8.7 4.7 16.7 12.3 20.9s16.8 4.1 24.3-.5l144-88c7.1-4.4 11.5-12.1 11.5-20.5s-4.4-16.1-11.5-20.5l-144-88c-7.4-4.5-16.7-4.7-24.3-.5z"/></svg></span> [Stanford CS224N: NLP with Deep Learning | Lecture 3 - Backpropagation, Neural Network](https://www.youtube.com/watch?v=HnliVHU2g9U&list=PLoROMvodv4rOaMFbaqxPDoLWjDaRAdP9D&index=3)*</span> is used to program the software 2.0 of deep learning. In this chapter, we will learn how to instantiate a neural network by selecting a specific instance of it's parameters by solving the optimization problem $\underset{\theta}{\operatorname{arg\,min}} \, \mathscr{L}(\theta)$.
...
> In which we delve deeper into foundations of gradient-based iterative optimization, building ourselves up to the AdamW optimizer.
...
Lorem ipsum<span class="lecnote">*<span class="fa-svg"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--! Font Awesome Free 6.2.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free --><path d="M512 256c0 141.4-114.6 256-256 256S0 397.4 0 256S114.6 0 256 0S512 114.6 512 256zM188.3 147.1c-7.6 4.2-12.3 12.3-12.3 20.9V344c0 8.7 4.7 16.7 12.3 20.9s16.8 4.1 24.3-.5l144-88c7.1-4.4 11.5-12.1 11.5-20.5s-4.4-16.1-11.5-20.5l-144-88c-7.4-4.5-16.7-4.7-24.3-.5z"/></svg></span> [MIT 6.7960 Deep Learning: Lec 07. Scaling Rules for Optimization](https://www.youtube.com/watch?v=VcGPE4s_oNw&list=PLUl4u3cNGP63URZnh5iqBzDTDYPUTQT-8&index=7)*</span> dolor sit amet, consectetur adipiscing elit. Proin finibus interdum arcu, ut venenatis dolor rutrum eget. Nulla nec urna tellus. Donec sollicitudin hendrerit sollicitudin. Proin sagittis, velit commodo congue rhoncus, metus turpis hendrerit nisi, a accumsan quam ante ultricies sem. Orci varius natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Ut aliquet ut justo non posuere. Quisque in rutrum nunc. Phasellus vehicula tortor mi, vehicula placerat diam ultrices eget. Aliquam et consequat eros, quis fermentum tortor. Sed eu nisl viverra, auctor velit vel, elementum ante. Orci varius natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Proin aliquet metus sit amet nulla cursus maximus in feugiat mi. Praesent vehicula magna sed scelerisque elementum. Nulla euismod rutrum turpis sed hendrerit.

</div>

<div class="full-bleed">
<iframe height="500px" loading="lazy" src="https://www.youtube.com/embed/BLkz5LGWihw?si=Uyk2WN3tcijUegl6" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
</div>

<div class="full-bleed">
<iframe height="500px" loading="lazy" src="https://www.youtube.com/embed/3d6DsjIBzJ4?si=8XYrvIfDLxLmj0jz" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
</div>
...
<span class="lecnote">*<span class="fa-svg"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--! Font Awesome Free 6.2.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free --><path d="M512 256c0 141.4-114.6 256-256 256S0 397.4 0 256S114.6 0 256 0S512 114.6 512 256zM188.3 147.1c-7.6 4.2-12.3 12.3-12.3 20.9V344c0 8.7 4.7 16.7 12.3 20.9s16.8 4.1 24.3-.5l144-88c7.1-4.4 11.5-12.1 11.5-20.5s-4.4-16.1-11.5-20.5l-144-88c-7.4-4.5-16.7-4.7-24.3-.5z"/></svg></span> [Stanford CS229 Machine Learning Lecture 14: Transformers, In-Context Learning](https://www.youtube.com/watch?v=pwQ0l4hFCVI&list=PLaqpC4kq8Gpw&index=15)*</span>

<span class="lecnote">*<span class="fa-svg"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--! Font Awesome Free 6.2.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free --><path d="M512 256c0 141.4-114.6 256-256 256S0 397.4 0 256S114.6 0 256 0S512 114.6 512 256zM188.3 147.1c-7.6 4.2-12.3 12.3-12.3 20.9V344c0 8.7 4.7 16.7 12.3 20.9s16.8 4.1 24.3-.5l144-88c7.1-4.4 11.5-12.1 11.5-20.5s-4.4-16.1-11.5-20.5l-144-88c-7.4-4.5-16.7-4.7-24.3-.5z"/></svg></span> [Stanford CME295 Transformers & LLMs | Lecture 1 - Transformer
](https://www.youtube.com/watch?v=Ub3GoFaUcds&list=PLoROMvodv4rOCXd21gf0CF4xr35yINeOy&index=3)*</span>

<span class="lecnote">*<span class="fa-svg"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--! Font Awesome Free 6.2.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free --><path d="M512 256c0 141.4-114.6 256-256 256S0 397.4 0 256S114.6 0 256 0S512 114.6 512 256zM188.3 147.1c-7.6 4.2-12.3 12.3-12.3 20.9V344c0 8.7 4.7 16.7 12.3 20.9s16.8 4.1 24.3-.5l144-88c7.1-4.4 11.5-12.1 11.5-20.5s-4.4-16.1-11.5-20.5l-144-88c-7.4-4.5-16.7-4.7-24.3-.5z"/></svg></span> [Stanford CS25: V2 I Introduction to Transformers w/ Andrej Karpathy](https://www.youtube.com/watch?v=MSfI6TTgyl4&list=PLoROMvodv4rOpr_A7B9SriE_iZmkanvUg&index=26)*</span>
...
- [Intermezzo Three: The Language of Neural Networks]()
  - [I3.1 Inner Product Spaces, Normed Spaces, Hilbert Spaces](#i31-derivatives-in-hilbert-spaces)
  - [I3.2 Sequences, Continuity, Limits, Derivatives](#i32-derivatives-in-hilbert-spaces)
  - [I3.3 Superposition]()
...
The Age of Research, roughly spanning the time period of 2012-2019,
...
let us briefly recall our journey that led us to this very moment.
...
are *evaluated* as next-token predictors $f: V^{*} \to V$ which although *decide* on a final classifying token $\mathbf{y} \in V$ for the given context $(\mathbf{x}_1,\mathbf{x}_2,\dots,\mathbf{x}_t)\in V^{*}$ (todo $V^{t}$?), have a step of *inference* by modeling language *stochastically* with a conditional random variable $Y|X_1,X_2,\dots,X_N$ endowed with a probability distribution $p_{Y|X_1,X_2,\dots,X_N}(\mathbf{y}|\mathbf{x}_1,\mathbf{x}_2,\dots,\mathbf{x}_t)$ where such distribution corresponds to an array $\mathbf{y} \in \reals^{V}$
which can be represented by a <img class="icon" src="./assets/borscht.png" alt="" style="aspect-ratio: 736 / 510;">[`borscht.Tensor`](https://github.com/j4orz/borscht) with `.shape` of `(V)`.
This was the first step in our transition from classical software 1.0 to the jazz of software 2.0,
namely, by transitioning from *variables* to *random variables* in [§1.1 From Certain to Uncertain Knowledge](./1.md#11-from-certain-to-uncertain-knowledge).
...
**automatic differentiation**<span class="defnote">**automatic differentiation**</span>
**optimizers**<span class="defnote">**optimizers**</span>
**network primitives**<span class="defnote">**network primitives**</span>
...
### 4.1 Gradient-Based Optimization with Automatic Differentation <!-- borscht.forward(), borscht.backward() -->
<small>[$\hookleftarrow$ Table of Contents](#ii-table-of-contents)</small>
...
- [4.1 Automatic Differentiation](#41-automatic-differentation)
  - [4.1.1 Symbolic, Numerical, and Algorithmic Differentiation](#411-symbolic-numerical-and-algorithmic-differentiation)
  - [4.1.2 Forward Mode with <img class="icon" src="./assets/borscht.png" alt="" style="aspect-ratio: 736 / 510;">`borscht.forward()`](#412-forward-mode-with-borschtforward)
  - [4.1.3 Backward Mode with <img class="icon" src="./assets/borscht.png" alt="" style="aspect-ratio: 736 / 510;">`borscht.backward()`](#413-backward-mode-with-borschtbackward)
  - [4.1.4 Stochastic Gradient Descent with <img class="icon" src="./assets/borscht.png" alt="" style="aspect-ratio: 736 / 510;">`borscht.optim.sgd()`]()
  - [4.1.5 Adam with <img class="icon" src="./assets/borscht.png" alt="" style="aspect-ratio: 736 / 510;">`borscht.optim.adamw()`]()
...
<span class="lecnote">*<span class="fa-svg"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--! Font Awesome Free 6.2.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free --><path d="M512 256c0 141.4-114.6 256-256 256S0 397.4 0 256S114.6 0 256 0S512 114.6 512 256zM188.3 147.1c-7.6 4.2-12.3 12.3-12.3 20.9V344c0 8.7 4.7 16.7 12.3 20.9s16.8 4.1 24.3-.5l144-88c7.1-4.4 11.5-12.1 11.5-20.5s-4.4-16.1-11.5-20.5l-144-88c-7.4-4.5-16.7-4.7-24.3-.5z"/></svg></span> [MIT 18.S096 Matrix Calculus for Machine Learning Lecture 5 Part 2: Forward Automatic Differentiation via Dual Numbers](https://www.youtube.com/watch?v=5F6roh4pmJU&list=PLUl4u3cNGP62EaLLH92E_VCN4izBKK6OE&index=12)*</span>

<span class="lecnote">*<span class="fa-svg"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--! Font Awesome Free 6.2.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free --><path d="M512 256c0 141.4-114.6 256-256 256S0 397.4 0 256S114.6 0 256 0S512 114.6 512 256zM188.3 147.1c-7.6 4.2-12.3 12.3-12.3 20.9V344c0 8.7 4.7 16.7 12.3 20.9s16.8 4.1 24.3-.5l144-88c7.1-4.4 11.5-12.1 11.5-20.5s-4.4-16.1-11.5-20.5l-144-88c-7.4-4.5-16.7-4.7-24.3-.5z"/></svg></span> [MIT 18.S096 Matrix Calculus for Machine Learning Lecture 5 Part 3: Differentiation on Computational Graphs](https://www.youtube.com/watch?v=r9_5dxtDTOk&list=PLUl4u3cNGP62EaLLH92E_VCN4izBKK6OE&index=12)*</span>

<span class="lecnote">*<span class="fa-svg"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--! Font Awesome Free 6.2.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free --><path d="M512 256c0 141.4-114.6 256-256 256S0 397.4 0 256S114.6 0 256 0S512 114.6 512 256zM188.3 147.1c-7.6 4.2-12.3 12.3-12.3 20.9V344c0 8.7 4.7 16.7 12.3 20.9s16.8 4.1 24.3-.5l144-88c7.1-4.4 11.5-12.1 11.5-20.5s-4.4-16.1-11.5-20.5l-144-88c-7.4-4.5-16.7-4.7-24.3-.5z"/></svg></span> [MIT 18.S096 Matrix Calculus for Machine Learning Lecture 8 Part 2: Automatic Differentiation on Computational Graphs](https://www.youtube.com/watch?v=43kl6wi1H6k&list=PLUl4u3cNGP62EaLLH92E_VCN4izBKK6OE&index=18)*</span>
...
<iframe  width="560" height="315" loading="lazy" src="https://www.youtube.com/embed/VkHfRKewkWw?si=ctIS6a1CxJrUBCxI" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
...
<iframe width="560" height="315" loading="lazy" src="https://www.youtube.com/embed/SmZmBKc7Lrs?si=OeqPIs5no9Fv1H_u" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
