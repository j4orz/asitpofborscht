...
- [3. Self-Supervised Sequence Learning with Deep Networks](#3-self-supervised-sequence-learning-with-deep-networks)
  - [3.1 Learning Representations with <span class="smallcaps">FFNs</span>](#31-learning-representations-with-ffns)
    - [3.1.1 From Machine Learning to Deep Learning](#311-from-machine-learning-to-deep-learning)
    - [3.1.2 From Localist to Distributional Representations](#312-from-localist-to-distributional-representations)
    - [3.1.3 From Linear Perceptrons to Non-Linear Multi Layer Perceptrons](#313-from-linear-perceptrons-to-non-linear-multi-layer-perceptrons)
    - [3.1.4 From Direct to Iterative Optimization](#314-from-direct-to-iterative-optimization)
...
Machine learning of software 2.0 serves as the foundation for training large language models.
Namely, the discipline of machine learning stochastically models language as [*next-token prediction*](./1.md#next-token-prediction), and estimates some [*classifier*](./1.md#classification) $\hat{f}: \mathscr{X} \to \mathscr{Y}$ that assigns an output token $y$ for a given input sentence $(x^{(1)}, \dots, x^{(t)})$ where $\mathscr{X} := V^{*}$ and $\mathscr{Y} := V$ from [*self-supervised*](./1.md#self-supervised-learning) data $d:=\{(\mathbf{x}^{(i)}, y^{(i)})\}_{i=1}^{n}$.
Moreover, regardless of whether $\hat{f}$ is a [*discriminative function*](./1.md#discriminant-functions), [*discriminative probabilistic model*](./1.md#discriminative-probabilistic-model), or [*generative probabilistic model*](./1.md#generative-probabilistic-model), it's modeled [*parametrically*](./1.md#parameterized) as $\hat{f}_{\theta}$,
...
namely, programming of software 2.0 with three step process of selecting a model architecture, scoring it with a loss, and optimizing such loss. However, deep learning focuses on the class of models $\hat{f}_{\theta}$ known as deep neural networks or multi-layer perceptrons that introduce many layers of non-linearities that make them well-suited for **representation learning**<span class="defnote">**representation learning**</span><span class="sidenote-number"></span><span class="sidenote">*See [(Bengio et al., 2012)](https://arxiv.org/pdf/1206.5538) and [(LeCun et al., 2015)](https://www.nature.com/articles/nature14539)*</span>. Namely, in [§3.1.2 From Localist to Distributional Representations](#312-from-localist-to-distributional-representations),
...
#### 3.1.3 From Linear Perceptrons to Non-Linear Multi Layer Perceptrons
<small>[$\hookleftarrow$ Table of Contents (§3.1 Learning Representations with FFNs)](#31-learning-representations-with-ffns)</small>
...
#### 3.1.4 From Direct to Iterative Optimization
<small>[$\hookleftarrow$ Table of Contents (§3.1 Learning Representations with FFNs)](#31-learning-representations-with-ffns)</small>
