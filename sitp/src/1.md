...
With us defining the notion of a joint probability as a multidimensional array, it's probably no surprise that the joint distribution $p_{\mathbf{w}}(\mathbf{w})$ corresponds to the `np.ndarray` with `.shape` `(V,...,V)` where `.ndim` is `t`. That is, `.shape` is a `t`-lengthed tuple.

<img class="tensors" src="./assets/tensors.png" alt="" style="display: block; margin: 0 auto;">



$p_{W,H}: V^{*} \times V \to [0,1]$
