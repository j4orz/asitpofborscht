...
So, starting from the string `"to"`, we still map the string to it's index $i$ in the vocabulary $V$ with some hash map $I: V \to \N$, but then we need to map the index $i \in \N$ to some vector $\mathbf{x} \in \reals^{|V|}$.
...
Because you are already familiar in how a final word decision $y$ gets chosen from  a distribution $p(y|x)$ with todo, let us pay attention to the newer components of the machinery, namely the vectorized input $\mathbf{x}$, the weighted sum of such input $\sum\limits_{i=1}^{d}w_i x_i$, and finally the squashing $\sigma(\cdot)$ of everything into a final probability scalar $y=p(y|x)$.
