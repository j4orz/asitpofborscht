...
- [2. The Multidimensional Parallelized Tensor](#2-the-massively-parallel-multidimensional-tensor)
  ...
  - [2.2 Accelerating Basic Linear Algebra on CPUs](#22-accelerating-matrix-multiplication-on-cpus) <!-- with computation and communication of rooflines -->
  ...
  - [2.4 Acceelerating Solvers with Decompositions]()
...
That if, even though software 2.0 shares the same mathematical tools of precision as software 1.0 in it's specification albeit continuous rather than discrete,
...
Throughout such transition you've become increasingly familiar with the different uses of the <img class="icon" src="./assets/borscht.png" alt="" style="aspect-ratio: 736 / 510;">[`borscht.Tensor`](https://github.com/j4orz/borscht) in programming continuous mathematics.
...
which we saw it's most general form in [§1.2 Next Token Prediction is Classification and Compression](#12-next-token-prediction-is-classification-and-compression) as a joint distribution of arbitrary random variables
...
which can be represented with a different uses of the <img class="icon" src="./assets/borscht.png" alt="" style="aspect-ratio: 736 / 510;">[`borscht.Tensor`](https://github.com/j4orz/borscht) whose `.shape` evaluates to `(d1, d2, ..., dn)`.
We saw that modeling a distribution over sentences became quickly intractable (todo, curse of dimensionality),
and let go of perfection by settling for the good by adding some bias. Namely, by approximating fully chained probabilities $p(w_1, \dots, w_i)$ for $i \in [1, t]$ with their fish-eyed conditionals $p(w_i|w_{i-n+1},\dots,w_{i-1})$ of n-lengthed ngrams via stochastic matrix `P_VV` whose `.shape` evaluated to `(V,V)`, in this case being `(26,26)`.
...
Moreover, after [§Intermezzo One: The Language of Continuous Mathematics](#intermezzo-one-the-language-of-continuous-mathematics), we understand these concepts more formally which is minimally useful by their *clarification of ambiguity* via precision and maximally by their *transferability of context* via generality — that a random variable of a the next token distribution $p_X(x)$ is a mapping $X: \Omega \to \reals$ from a **probability space** $(\Omega, \mathcal{F}, \mathbb{P})$ to another measurable space, and how language models are
...
At this point the mathematicians are reading and nodding along with joy.
But for better and for worse, we must slightly view these mathematical objects under a new light that is more computational.
...
<small>[$\hookleftarrow$ Table of Contents](#i-table-of-contents)</small><span class="gutter-figure gutter-figure-left" style="--margin-note-raise: 48px"><img src="./assets/iverson.png" alt="Kenneth E. Iverson at the blackboard"><span class="gutter-figure-caption">[Kenneth E. Iverson](https://en.wikipedia.org/wiki/Kenneth_E._Iverson)</span></span><span class="gutter-figure gutter-figure-right"><img src="./assets/iverson_book.png" alt="Iverson's 1962 A Programming Language"><span class="gutter-figure-caption">[*A Programming Language* (1962)](https://archive.org/details/aprogramminglanguage1962)</span></span>
...
- [2.1 Virtualizing Shapes with Strides]()
  ...
  - [2.1.5 Implementing Layer 0: `saxpy`, `sgemv` and `sgemm`]()
