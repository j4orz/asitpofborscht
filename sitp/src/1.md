...
With a better understanding of the machine learning activity of modeling language as a probabilistic phenomena and estimating some classifying function $\hat{f}: \mathscr{X} \to \mathscr{Y}$ where $\mathscr{X} := V$ and $\mathscr{Y} := V^{*}$ (todo, should Y just be V because of the bigram Markov assumption?) from self-supervised data $d:=\{(\mathbf{x}^{(i)}, y^{(i)})\}_{i=1}^{n}$, let us now introduce a different way of estimating $\hat{f}$, namely by learning a set of **parameters**<span class="defnote">**parameters**</span> including  **weights**<span class="defnote">**weights**</span> and **biases**<span class="defnote">**bias**</span> (which comprise a single layer of a deep neural net) from data.
...
<img class="neuron" src="./assets/neuron.png" alt="" style="display: block; margin: 0 auto;">
