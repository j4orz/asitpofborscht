...
- [1. Self-Supervised Sequence Learning with Single-Layer Networks](#1-sequence-learning)
  ...
  - [1.3 Parameterizing Classification](#13-parameterizing-classifcation)
  - [1.4 Parameterizing Quantification]() <!-- with least squared error -->
  - [1.5 Summary](#16-summary)
  - [1.6 Further Reading](#17-further-reading)
  - [1.7 Problems](#18-problems)
- [Intermezzo One: The Language of Probability, Linear Algebra, and Calculus](#intermezzo-one-the-language-of-probability-theory-and-linear-algebra)
...
- [2. The Massively Parallel Multidimensional Tensor](#2-the-multidimensional-tensor)
  ...
  - [2.3 Accelerating Matrix Multiplication]() <!-- with computation and communication of rooflines -->
  - [2.4 Accelerating Matrix Multiplication Redux]() <!-- simd to simt -->
  - [2.5 Accelerating Matrix Multiplication Redux 2]() <!-- gpu to tpu -->
...
which is called a **distribution**<span class="defnote">**distribution**</span>,
because it is *distributing* truth across a weighted set of values,
on in other words, it's **uncertainty**<span class="defnote">**uncertainty**</span>.
...
such a distribution is a **conditional distribution**<span class="defnote">**conditional distribution**</span>. That is, GPT-2 is producing the distribution of the next word conditioned on the sequence of words passed in as input, and is denoted by
...
and $\sum\limits_{n \in \N}=1$. That is, any list of values between 0-1 and normalizes to 1 is a valid distribution.
...
- [1.2 Next Token Prediction is Classification](#12-next-token-prediction-is-classification)
  ...
  - [1.2.2 Training and Inference with Maximum Likelihood Estimation]()
  - [1.2.3 Intelligence as Compression]()
...
GPT-2 produces a conditional distribution with an `ndarray` of `.shape` `(V,)` which represents a vector with support in $\reals^{|V|}$
— since we are only taking the top 10 most likely words though, `.shape` is `(10,)` with support in $\reals^{10}$.
We now shift our attention from *using* language models to *implementing* ones, justl ike GPT-2 that can produce such a conditional distribution $p(w|\mathbf{h})$.
...
Once a distribution like $p(w|\mathbf{h})$ is estimated from data by research labs,
...
$d:=\{(\mathbf{x}^{(i)}, \mathbf{y}^{(i)})\}_{i=1}^{n}$, then the regime is known to be **supervised learning**<span class="defnote">**supervised learning**</span>, for the machine that is trying to learn a distribution via induction is being *supervised* with the expected outputs, whereas the regime where such data is unavailable is known as **unsupervised learning**<span class="defnote">**unsupervised learning**</span>.
...
Now that we understand the process of language modelling is to estimate a classifying function $f: V^{*} \to V$ under a self-supervised setting with data $d := \{(\mathbf{x}^{(i)}, y^{(i)})\}_{i=1}^{n}$,
how do we go about actually estimating the distribution $p_{W|H}: V^{*} \to \Delta^{|V|}$ to compose with a decision boundary (todo) resulting in $f$?
...
#### 1.2.4 Evaluation with Bias-Variance Tradeoff
...
<img class="stochastic_matrix" src="./assets/stochastic_matrix.png" alt="">
...
With the plot in stdout is a heatmap of the first five $\mathbf{x}^{(i)}$'s.
Continuing our left to right scan of the neuronal circuit in Figure 1.3.1,
the next pieces of machinery found in the logistic regression model after tensorizing integer indices to their one hot vectors is passing such input into a weighted sum $\sum\limits_{i=1}^{|V|}w_i x_i$ and squashing function $\sigma(\cdot)$. All together, we have that $\hat{f} := \sigma(\sum\limits_{i=1}^{|V|}w_i x_i)$.

In the case of logistic regressoin, that means assigning
$\hat{f} := \sigma(\sum_{i=1}^{d} w_i x_i)$
...
- stochastic matrix is multi-class classification. neuron is binary classification (sigmoid, not softmax)
- code has a single layer of V neurons (stack of logistic regressions)
...
f:V &\to V\\
v &\mapsto \text{rng} \circ P \circ I(v)
...
f: V &\to V\\
   v &\mapsto \text{rng} \circ \sigma(\mathbf{w}^{\top}(v) + b)
...
<!-- with parameters $\theta := \{\mathbf{w}, b\}$ where $\mathbf{w} \in \reals^d$ are referred to as **weights**<span class="defnote">**weights**</span> and $b \in \reals$ which is known as the **bias**<span class="defnote">**bias**</span>. These weights and biases comprise a single layer $h_{i}$ of a deep neural network. -->
...
Recall in [§1.2 Next Token Prediction is Classification](#12-next-token-prediction-is-classification) that a bigram model was used to model language, namely, a stochastic matrix with support in $\reals^{V \times V}$ (and whose rows normalize to 1 with values between 0 and 1) corresponding to `P_VV`, a `np.ndarray` with a `.shape`  of `(V,V)`, which in the case of modeling language at the level of characters happens to be `(27,27)`. Because the stochastic matrix $\reals^{V \times V}$ is viewed as a function $P: \N \times \N \to \reals$ that maps indices $(i,j) \in |V| \times |V|$ to their conditional probabilities $p(w_i|h_i)\in \reals$ (which can be seen as indexing `P_VV` with `P_VV[(i,j)]`),

1. since the classifying function is $f: V \to V$, when passing history input $v \in V$ to curry $P: \N \times \N \to \reals$ which plucks some row $p(w|h)$, $v$ needed to be mapped from $V \to \N$ first with the `c2i` dict.
2. moreover, after the row distribution $p(w|h) \in \reals^{|V|}$ is plucked, a decision needs to be made in order to select one specific $v \in V$, in which `np.random.choice` was used
...
### 1.4 Parameterization Redux: Quantification  <!-- with Least Squared Error -->
<small>[$\hookleftarrow$ Table of Contents](#table-of-contents)</small>
...
## Intermezzo One: The Language of Probability, Linear Algebra, and Calculus
<small>[$\hookleftarrow$ Table of Contents](#table-of-contents)</small>
...
- [2. The Massively Parallel Multidimensional Tensor](#2-from-ipls-array-to-apls-multidimensional-array)
  ...
  - [2.3 Accelerating Matrix Multiplication with Multicore CPUs]() <!-- with computation and communication of rooflines -->
  - [2.4 Accelerating Matrix Multiplication with Manycore GPUs]() <!-- simd to simt -->
  - [2.5 Accelerating Matrix Multiplication with Tensorcore TPUs]() <!-- gpu to tpu -->
  - [2.6 From Strides to Layouts]()
...
### 2.3 Accelerating Matrix Multiplication
<small>[$\hookleftarrow$ Table of Contents (§2. The Multidimensional Tensor)](#2-the-multidimensional-tensor)</small>
...
### 2.4 Accelerating Matrix Multiplication Redux
<small>[$\hookleftarrow$ Table of Contents (§2. The Multidimensional Tensor)](#2-the-multidimensional-tensor)</small>

### 2.5 Accelerating Matrix Multiplication Redux 2
<small>[$\hookleftarrow$ Table of Contents (§2. The Multidimensional Tensor)](#2-the-multidimensional-tensor)</small>
