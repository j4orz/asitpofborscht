...
- [2.1 Virtualizing Shapes with Strides]()
  - [2.1.1 From Virtual to Physical Machines](#211-from-virtual-to-physical-machines)
  - [2.1.2 The Architecture of a Deep Learning Framework](#212-implementing-layer--1-a-stride-maps-a-logical-shape-to-a-physical-storage)
...
that have a **high level**<span class="defnote">**high level**</span><span class="sidenote-number"></span><span class="sidenote">*Semantics...*</span> veneer for *productivity*
with a **low level**<span class="defnote">**low level**</span> internal implementation for *performance*
where, roughly speaking, high level refers to languages such as Python that are closer to the English language that humans speak, whereas low level refers to those such as C++ or Rust that are closer to the machine languages that electricity "speaks". The process of computation, at the end of the day, is the orchestration of electrons. This chasm from English electricity is referred to as the **semantic gap**<span class="defnote">**semantic gap**</span>.
...
interpret MIR vs compile to risc-v,
**virtual machine**<span class="defnote">**virtual machine**</span> 
referring to compilers that target machine languages as **native**<span class="defnote">**native**</span> language because such a program will be executed by a **physical machine**<span class="defnote">**physical machine**</span><span class="sidenote-number"></span><span class="sidenote">*In the sciences of artificial, all distinctions are usually spectrums. Languages such as Java, Javascript, and soon to be Python can by default be interpreted by a virtual machine, and then just-in-time compiled to a physical machine.*</span>. And because the primary implementation of the Rust programming language is the compiler `rustc` which targets architectures such as `x86`, `ARM`, and `RISC-V`<span class="sidenote-number"></span><span class="sidenote">*Actually, `rustc` actually targets a language called `LLVM`, which then gets translated to the three aforementioned assembly languages. It's turtles all the way down.*</span>, the Rust programming language like C and C++, are often referred to as a "low-level" and "native".

In fact, the Python language is aware of how important natively accelerated software is for numerical computing
that they've added support to Python's reference interpreter CPython with the ability to extend the virtual Python interpreter
with native C/C++ code that is compiled to assembly and executed by a physical processor.

Although Rust is similar to Python in that is has support for structured programming<span class="sidenote-number"></span><span class="sidenote">*See [Structured Programming (Dahl, Dijkstra, Hoare 1972).](https://archive.org/details/Structured_Programming__Dahl_Dijkstra_Hoare)*</span> with [control flow](https://rust-book.cs.brown.edu/ch03-05-control-flow.html) and [functions](https://rust-book.cs.brown.edu/ch03-03-how-functions-work.html), in contrast to Python, Rust is a **statically typed**<span class="defnote">**statically typed**</span> programming language in that it has [variables](https://rust-book.cs.brown.edu/ch03-01-variables-and-mutability.html) whose types must be known at [compile time](https://rust-book.cs.brown.edu/ch03-02-data-types.html) before the program is executed. What differentiates Rust from other languages though is it's unique combination of two language features that change the way we program *data*, **ownership**<span class="defnote">**ownership**</span> that comes **systems programming**<span class="defnote">**systems programming**</span> and **algebraic data types**<span class="defnote">**algebraic data types**</span> that come from  **functional programming**<span class="defnote">**functional programming**</span>.
We will explain these two language features by their use.


We will briefly cover these two before moving on to [§2.1.2 The Architecture of a Deep Learning Framework](#212-implementing-layer--1-a-stride-maps-a-logical-shape-to-a-physical-storage), but to learn or revisit Rust from the ground up, please consult Brown's fork of The Rust Programming Language<span class="sidenote-number"></span><span class="sidenote">*See [Will Crichton: Rust for Everyone!
...
So with that all said and done, let us create a mixed source of Python and Rust for <img class="icon" src="./assets/borscht.png" alt="" style="aspect-ratio: 736 / 510;">[`borscht`](https://github.com/j4orz/borscht).

First, get [yourself an installation](https://rust-lang.org/tools/install/) of the Rust **toolchain**<span class="defnote">**toolchain**</span>
specific to your operating system architecture. Such a toolchain includes the reference **compiler**<span class="defnote">**compiler**</span> `rustc` which translates your program from the Rust language to machine language, the **package manager**<span class="defnote">**package manager**</span> `cargo` to drive `rustc` on your behalf, and the **version manager**<span class="defnote">**version manager**</span> `rustup` to manage different versions of Rust implemented by `rustc`. This is in contrast to Python's reference interpreter [`CPython`](https://devguide.python.org/internals/) which evaluate your Python program directly and it's package manger [`uv`](https://docs.astral.sh/uv/).
...
```sh
> root@machine ~/ $ maturin new --mixed borscht
✔ 🤷 Which kind of bindings to use?
...
```sh
> root@machine ~/borscht $ source .venv/bin/activate
...
```sh
> (borscht) root@machine borscht $ uv pip list
...
- [2.3 Accelerating Matrix Multiplication on CPUs]()
  ...
  - [2.3.3 Measuring Bottlenecks to Throughput with Roofline Plots]()
