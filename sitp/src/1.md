...
- [1. Self-Supervised Sequence Learning with Single-Layer Networks](#1-sequence-learning)
  ...
  - [1.2 Estimation by Counting Bigrams](#12-estimation-by-counting-bigrams)
  - [1.3 Categorical Parameterization with Logistic Classification](#13-parameterization-by-fitting-logistic-regression) <!-- logistic reg/1 layer ffn -->
...
> In which we transition to the stochastic and infinitely continuous software 2.0 with ngram and linear models using probability theory, linear algebra, and calculus.
...
even with a set of initial equations for position and momentum with equations for change
,<span class="defnote">**bitter lesson**</span>**describing reality with too many parts to count**.
...
But rather the  n produce a distribution with two outcomes like a coin,
...
and with Demo 1.1, you are asking GPT-2 to produce $p(w| \text{Hello nice to})$.
...
We can print the conditional distribution that GPT-2 produces given the history $\text{Hello nice to}$
...
print(f"Total sum of p(w|Hello nice to): {probs.sum():.4f}")
```
...
> [!IMPORTANT]
> <img class="owl" src="./assets/owl.png" alt="" style="float: left; width: 100px; margin: 0 1em 0.5em 0;"> **Probability can be seen as array programming**! Any function $f: \reals \to [0,1]$ or equivalently `np.ndarray` whose values are between 0 and 1 and normalizes to 1 is a valid distribution.<br style="clear: both;">
...
in this case
$\mathbf{h} \in \reals^3$ where $\mathbf{h}_0=\text{Hello}$, $\mathbf{h}_1=\text{nice}$, $\mathbf{h}_2=\text{to}$.
...
### 1.2 Estimation by Counting Bigrams
<small>[$\hookleftarrow$ Table of Contents](#table-of-contents)</small>
...
For instance, with the input text `"Hello nice to meet"` as the history $\mathbf{h}$,
...
we used GPT-2 to produce a conditional distribution $p(w|\text{Hello nice to})$ whose next-token prediction capability
...
```python
import numpy as np
history_VVVV = np.array_with_shape((V, V, V, V, V, V)) # t=4, i.e "Hello nice to"
joint_VVVVV = np.array_with_shape((V, V, V, V, V, V)) # t=5, i.e p("Hello nice to") AND p("you") 
...
<div class="full-bleed">
<video src="./assets/entropy1.webm" controls></video>
</div>


> [!IMPORTANT]
> <img class="owl" src="./assets/owl.png" alt="" style="float: left; width: 100px; margin: 0 1em 0.5em 0;"> we don't enumerate through the entire sample space nor event space,
...
### 1.3 Parameterization by Fitting Logistic Regression  <!-- with Cross Entropy Loss $\mathscr{L(\theta)} := $ -->
<small>[$\hookleftarrow$ Table of Contents](#table-of-contents)</small>
...
### 1.4 Lagniappe: Fitting Linear Regression  <!-- with Least Squared Error -->
<small>[$\hookleftarrow$ Table of Contents](#table-of-contents)</small>
...
GPT-2's `np.ndarray` output `probs` when passed the input string `"Hello nice to"`:

```python
import numpy as np

# GPT-2's output distribution when passed input "Hello nice to"
