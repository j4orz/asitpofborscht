...
> [!TIP]
> <img class="owl" src="./assets/owl.png" alt="" style="float: left; width: 100px; margin: 0 1em 0.5em 0;"> Try out the Green theme for the following systems chapter. <br style="clear: both;">
...
> In which we introduce the so-called three language problem that deep learning frameworks suffer from,  which introduce accelerated languages for the host and device with Rust and CUDA Rust, introduce the Rust programming language, and finally, set up a mixed source project of Python and Rust for <img class="icon" src="./assets/borscht.png" alt="" style="aspect-ratio: 736 / 510;">[`borscht`](https://github.com/j4orz/borscht) with `PyO3` and `maturin`.
...
If we `cd` into `borscht` and `ls` with the `-l` flag we see two `.toml` files created for us, with two directories of Python and Rust source respectively:
...
Opening the two configuration files of `pyproject.toml` and `Cargo.toml` side by side
