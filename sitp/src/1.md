...
which is viewing vectors from the data-perspective. For instance `probs` being some vector $v \in \reals^{10}$.
But recall viewing the vector `probs` from the function-perspective, is it some function $p: \N \to \reals$.
