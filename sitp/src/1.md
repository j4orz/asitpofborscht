...
and thus correspond to a *distributed* array of values which we call a *distribution*.
We can print the distribution that GPT-2 produces given the history $\text{Hello world, nice to}$
...
where [`np.argsort`](https://numpy.org/doc/stable/reference/generated/numpy.argsort.html)

Every random variable is endowed with a distribution, which distributes the truth or state of the random variable (in this case the next word) across a vocabulary of 50257 words.
...
and the type of a distribution $p$ is some function $p: \N \to [0,1]$
that sends indices to their probabilities, such that for all $n\in \N, p(n) \ge 0$
and $\sum\limits_{n \in \N}=1$.


We can validate that such a distribution $p(w|\mathbf{h})$ is valid
...
(todo: **probability mass function**<span class="defnote">**probability mass function**</span>, **probability density function**<span class="defnote">**probability density function**</span>).
...
> In which we introduce the terminology of software 2.0 in the context of language modeling, implement our first language model with biased histograms following the work of Andrey Markov and Claude Shannon in the early 20th century, evaluate said model with information theoretical quantities such as perplexity and entropy, and understand intelligence more generally as compression.
...
Although we know that it has the type $p_{W|H}: V^{*} \to V$, what is the *definition* of it's function body?
