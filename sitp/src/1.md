...
A circuit for the **linear regression**<span class="defnote">**linear regression**</span> model is shown on the right hand side of Figure 1.3.1. Rather than produce the <span class="okabe-orange">orange output $p(y|\mathbf{x})$</span> with a stochastic matrix containing normalized relative frequency counts, the linear regression model produces it by passing the <span class="okabe-blue">blue input $\mathbf{x}$</span> into the <span class="okabe-green">green summing function $\sum$</span> with the sum being configured — in this case weighted — with <span class="okabe-purple">purple parameters $w$ and $b$</span>.
Intuitively speaking, by *parameterizing* $\hat{f}$ what we mean is that we will *assume* or *model* $\hat{f}$ as some function that has configurable parameters. You can view the linear regression model on the right as adding some indirection between the blue input and orange output with green machinery configured by purple parameters. The problem of training a good language model then reduces to that of finding a good set of parameters, in this case $\theta := \{\mathbf{w}, b \}$.
...
That different approach, is with the **weights**<span class="defnote">**weights**</span> $w_i$ and **biases**<span class="defnote">**bias**</span> $b$ of a **linear regression**<span class="defnote">**linear regression**</span> model which can also be viewed as a single **neuron**<span class="defnote">**neuron**</span> from a deep net.
...
In terms of a desirata for the *decision* step, that is assigning the input some outcome, it would be nice if we could divide the plane into two **decision regions**<span class="defnote">**decision regions**</span> $\cout{\mathcal{R_1}}$ and $\cout{\mathcal{R_2}}$ with some line $y$ as a **decision boundary**<span class="defnote">**decision boundary**</span> $\cout{y}$ such that our classifier assigned the next word $w_{t+1}$ to be $\text{york}$ given $\cin{(1,0)^{\top}}$ for $\cin{\text{new}}$, and perhaps $\cin{(1,0)^{\top}}$ for $\cin{\text{york}}$. You can see how a 2-worded vocabulary easily breaks down — we'll be increasing our vocabulary size momentarily — but with such a simplified example, we can clearly see the the decision boundary $y$ on the left does not separate the two input vectors whereas the one on the right does.
While the one-hot input $\cin{(1,0)^{\top}}$ for $\cin{\text{new}}$ is correctly assigned the decision region $\cout{\mathcal{R_1}}$ for york,
the other input $\cin{(1,0)^{\top}}$ for $\cin{\text{york}}$ is not correctly assigned the decision region $\cout{\mathcal{R_2}}$ for york, but rather $\cout{\mathcal{R_1}}$ as well.
(remove w, and also change one blue circle to be an x)
...
Given that $\cparam{\mathbf{w}}$ is the only term left from the circuit not mentioned,
you may have already surmised that such a decision boundary is determined by the weight vector $\cparam{\mathbf{w}}$, summed with the input vector $\cin{\mathbf{x}}$ with a weighted sum $\cfun{\sum}$. That is,

$$
\cout{y} = \cfun{\sum \limits_{i=1}^{d}}\cparam{w_i} \cin{x_i}
$$
...
*If* we can find a decision boundary $y$,
