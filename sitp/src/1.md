...
- [2. The Massively Parallel Multidimensional Tensor](#2-the-multidimensional-tensor)
  ...
  - [2.4 Accelerating Matrix Multiplication on GPUs](#24-accelerating-matrix-multiplication-on-gpus) <!-- simd to simt -->
  - [2.5 Accelerating Matrix Multiplication on TPUs](#25-accelerating-matrix-multiplication-on-tpus) <!-- gpu to tpu -->
...
> In which we have our first encounter with language modeling via GPT-2 and teenygrad, and the foundational notions from probability and linear algebra used, namely random variables, their distributions, and their computational equivalent of the multidimensional array.
...
Large language models are trained using methods from the discipline of **deep learning**<span class="defnote">**deep learning**</span>,
which in turn, are based in the statistical **machine learning**<span class="defnote">**machine learning**</span> approach to artificial intelligence.
...
is the chance that GPT-2 assigns a **random variable**<span class="defnote">**random variable**</span>
...
with each `probs[i]` corresponding to the chance, or possibility, of the random variable taking on the value `tokens[i]`. That is, what connects the weighted array `tokens` and the state space `probs` is the index `i`,
and the type of a distribution $p$ then is some function $p: \N \to [0,1]$
...
todo: start iteratively refining concept of vectors. maybe start second rung when getting into representation learning of distributional representations. one-hot vectors are still localist.
...
which constructs the **multidimensional array**<span class="defnote">**multidimensional array**</span> [`np.ndarray`](https://numpy.org/doc/stable/reference/generated/numpy.ndarray.html).
While we will gradually become more intimately familiar with multidimensional arrays such as `np.ndarray` throughout the course of this adventure,
as a first approximation multidimensional array's are simply what they say on the tin can.
That is, they are arrays with multiple **dimensions**<span class="defnote">**rank**</span>,
enabling the representation of **scalars**<span class="defnote">**scalar**</span>, **vectors**<span class="defnote">**vector**</span>, **matrices**<span class="defnote">**matrix**</span>, and arbitrary **tensors**<span class="defnote">**tensor**</span> of arbitrary rank.
...
Where `probs.shape`<span class="defnote">**`.shape`**</span> and `probs.dtype`<span class="defnote">**`.dtype`**</span> evaluating to `(10,)` and `dtype64` respectively means
...
<img class="vector" src="./assets/vector.png" alt="" style="display: block; margin: 0 auto;">
...
> In which we introduce the terminology of software 2.0 in the context of language modeling; implement our first language model by maximizing the likelihood of biased ngrams; evaluate said model and it's limits with information theoretical and probabilistic quantities such as perplexity, entropy, bias variance tradeoff, and the curse of dimensionality; and finally, understand intelligence more generally as compression.
...
- [1.2 Next Token Prediction is Classification](#12-next-token-prediction-is-classification)
  ...
  - [1.2.2 Counting Markov Biased Histograms](#122-counting-markov-biased-histograms)
  - [1.2.3 Some More on Maximum Likelihood Estimation](#133-optimizing-loss-with-iterative-reweighted-least-squares)
  - [1.2.3 Prediction is Compression is Intelligence]()
  - [1.2.4 Evaluation with Perplexity, and Bias-Variance Tradeoff]()
  - [1.2.5 Further Viewing and Reading]()
...
As mentioned in the previous chapter, large language models are trained using methods from the discipline of **deep learning**<span class="defnote">**deep learning**</span>,
which in turn are based in **machine learning**<span class="defnote">**machine learning**</span>,
...
that predicts the expected output given the input. For instance, with the following supervised dataset:
...
But how do we reconcile the goal of estimating some function $f: V^{*} \to V$, when GPT-2 is the distribution $p(w|\mathbf{h})$
and produces a function of type $p_W: \N \to [0,1]$? Recall that the distribution $p: \N \to [0,1]$ over possible next words is the *output* of GPT-2.
...
And the corresponding *input* was $h=\text{Hello word, nice to}$.
Then, the type of $p_{W|H}(w|\mathbf{h})$ can be viewed as $p_{W|H}: V^{*} \to [0,1]^{|V|}$,
where $[0,1]^{|V|}$ can be seen as the set of all functions $\{ f: \N \to [0,1] \}$.
(todo, $\Delta^{|V|}$, which refines the type of the codomain to be the probability simplex satisfying non-negativity and normalization.)
Finally, composing $p_{W|H}$ with a decision boundary (todo) results in one final output which is an element of $V$.
Then, GPT-2, or next-token prediction language models more generally, are functions $f: V^{*} \to V$.
We already intuitively understood this with the notion that language models are next-token predictors.

- You can pass any sentence as input to GPT-2 and it will produce a next-word distribution. (todo, context-length limits this) (todo. words vs tokens.)
- (todo) **autoregressive model**<span class="defnote">**autoregressive model**</span>
...
In the case of a *categorical* output, the supervised learning is known as **classification**<span class="defnote">**classification**</span> for the machine's goal (the estimated function's output) is trying to classify the output's category.
...
This stands in contrast to the case when the output the estimated function is predicting is *quantitative*, which is known as **regression**<span class="defnote">**regression**</span>.
...
- If the type of the output is infintely continuous like $\reals$, then the supervised learning is said to be of **regression**<span class="defnote">**regression**</span>, which stands in contrast to when the type of output is discretely finite like $\N$, in which supervised learning is said considered to be of
...
and **responses**<span class="defnote">**responses**</span> in the context of ___.
...
#### 1.2.2 Counting Markov Biased Histograms
...
Now that we understand the process of language modelling is to estimate a classifying function $f: V^{*} \to V$ under a self-supervised setting with access to input-output paired data $d := \{(\mathbf{x}^{(i)}, y^{(i)})\}_{i=1}^{n}$,
how do we actually go about estimating the distribution $p(w|h)$?
We know that $p_{W|H}(w|h)$ *denotes* the conditional distribution of the next word given the history of sentence thus far, we know this distribution *corresponds* to a `np.ndarray` with `.shape` `(V)`, and we know the function *type* is $p_{W|H}: V^{*} \to \Delta^{|V|}$ (todo: $\Delta^{|V|}$ can be viewed as $|V| \to \reals$) which maps sentences to distributions over next words, but what is the actual function *body* or *definition* of a conditional distribution?
...
Let us first start by generalizing the notion of a distribution from a *flat* array of real-valued numbers satisfying non-negativity and normalization to that of a *multidimensional* array with the same kind of values, known as a
**joint distribution**<span class="defnote" data-refine="distribution">**joint distribution**</span> denoted by $p_{X_1, \dots, X_t}$ with type $p_{X_1, \dots, X_t}: \reals \times \cdots \times \reals \to [0,1]$ which maps the values of $t$ random variables to their probabilities<span class="sidenote-number"></span><span class="sidenote">*As we will see shortly, we use $t$ for time since language has a sequential order, given that $n$ already denotes the length of the dataset $d:=\{(x^{(i)}, y^{(i)})\}_{i=0}^{n}$. Naming is hard!*</span>. That is, evaluating $p_{X_1, \dots, X_t}(x_1, \dots, x_t)$ produces a *single* probability for random variable $X_1$ taking on the value $x_1$ *and* $X_2$ taking on the value $x_2$ *and* so on, up until the random variable $X_t$ taking on the value $x_t$. Because the joint can be a distribution of two random variables, three, up to an arbitrary t, it's often vectorized as $p_{\mathbf{X}}(\mathbf{x})$.

In the case of (probabilistic) language modeling, the primary joint distribution of interest is the probability of a *fixed* t lengthed sentence occuring, which is $p_{W1,W2,\dots,W_t}(w_1,w_2,\dots,w_t)$ or $p(\mathbf{w})$  with $w \in \reals^{t}$, where the $W_i$'th random variable represents the $i$'th word in the $t$-lengthed sentence. For instance, $p(w_1=\text{Hello}, w_2=\text{nice}, w_3=\text{to}, w_4=\text{meet}, w_5=\text{you})$ is the probability of the complete sentence `"Hello nice to meet you"`.

> [!WARNING]
> <img class="owl" src="./assets/owl.png" alt="" style="float: left; width: 100px; margin: 0 1em 0.5em 0;"> Note with the random variables being modeled the way they are (that is, *with* order) that the joint $p(w_1,\dots, w_t)$ is commutative. For instance, with $n:=2$, $p(w_1, w_2) = p(w_2,w_1)$. For instance, semantically speaking, $p(w1=\text{Hello},w_2=\text{world})=p(w_2=\text{world}, w1=\text{Hello})$ is saying that the probability of the first word being "hello" and the second word being "world" is equal to the probability of the second word being "world" and the first word being "hello". This holds more generally for any permutation of $w_1, w_2, \dots, w_t$. <br style="clear: both;">

With us defining the notion of a joint probability as a multidimensional array, it's probably no surprise that the joint distribution $p_{\mathbf{w}}(\mathbf{w})$ corresponds to a `np.ndarray` with `.shape` `(V,...,V)` where `.ndim` is `t`. That is, `.shape` is a `t`-lengthed tuple. Take a look at Figure 1.2.2 below:

<img class="tensors" src="./assets/tensors.png" alt="" style="display: block; margin: 0 auto;">

Figure 1.2.2<span class="sidenote-number"></span><span class="sidenote">*Alas, we failed to not use $n$ here. This $n$ is not the same n as the length of the dataset.*</span>

In particular, let us consider the joint distribution of two random variables $p_{X,Y}$, whose 2 dimensional surface can be plotted. This distribution has type $p_{X,Y}: \N \times \N \to \reals$, which corresponds to a `np.ndarray` with a `.shape` of `(n,m)` — a **matrix**<span class="defnote">**matrix**</span> supported in $\reals^{n \times m}$, or in other words, a $n$ by $m$ *rectangular* array that maps indices in $\{0,1,\dots,n\} \times \{0,1,\dots, m\}$ to their values in $\reals$.
...
The joint of three random variables $p_{X,Y,Z}: \N \times \N \times \N \to \reals$ is plotted as what is known as a voxel plot (it's difficult to see 4 dimensions alltogether), which corresponds to a `np.ndarray` with a `.shape` of `(n,m,p)` — a **tensor**<span class="defnote">**tensor**</span> supported in $\reals^{n \times m}$, a *cubic* array that maps indices in $\{0,1,\dots,n\} \times \{0,1,\dots, m\} \times \{0,1,\dots, p\}$ to their values in $\reals$. (todo, remark on naming with tensor.)
Similarly, the tensor is initialized with `np.zeros((n,m,p))`, is filled with values, and is plotted with `ax.voxels(x)`.
**Probability can be viewed as array programming!**

Returning to joint distribution of interest to language modeling, namely the probability of a sentence taking on specific words $p(w_1, w_2, \dots, w_t)$, let $t:=2$ so that we can consider the joint of two random variables $p(w_1, w_2)$,
corresponding to a matrix represented by a `ndarray` with `.shape` of `(V,V)`.
The values of the matrix in Figure 1.2.3 is one *possible* assignment to the `ndarray`, or equally, one *possible* assignment to the function body $p_{W_1 \times W_2}: \N \times \N \to \reals$.

<img class="joint" src="./assets/joint.png" alt="" style="display: block; margin: 0 auto;">
...
And because Figure I.2 is a table of numbers, it is also an `np.ndarray` with `.shape` `(V,V)`,
where marginalizing is `.sum()` along an axis and conditioning is a division:

<!-- {{#nb 1.2_bigram.ipynb:8}}

{{#nb 1.2_bigram.ipynb:9}}

A table is not the only way to see it. Where a distribution over *one* variable plots as a
curve over its state space — the bar chart of GPT-2's next-word distribution in
[§1.1](#11-from-certain-to-uncertain-knowledge) — a distribution over *two* plots as a curve over a two dimensional
state space, which is to say a surface. Conditioning on $\cin{H}=\cin{h}$ is then a *cut* through
that surface: fix $w_{t-1}$, sweep $w_t$, and read off the height. What you get back is a curve
over one variable again, and dividing it by $p_{\cin{H}}(\cin{h})$ is what makes it stand on its
own as a distribution:

{{#nb 1.2_bigram.ipynb:10}} -->


<span class="defnote" data-refine="conditional-distribution">**conditional distribution**</span>?

$$
\underbrace{p_{\cout{W}|\cin{H}}(\cout{w}|\cin{h})}_{\text{conditional distribution}}\stackrel{\triangle}{=} \frac{\overbrace{p_{\cout{W},\cin{H}}(\cout{w},\cin{h})}^{\text{joint distribution}}} {\underbrace{p_{\cin{H}}(\cin{h})}_{\text{marginal distribution}}}
$$

so, the conditional is defined as the joint over the marginal distribution that is being conditioned on.
...
#### 1.2.3 Some More on Maximum Likelihood Estimation
...
#### 1.2.4 Prediction is Compression is Intelligence

<div class="full-bleed">
<iframe height="500px" loading="lazy" src="https://www.youtube.com/embed/l6DKRf-fAAM?si=4Ee7_pRjZfLteRs2"  title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
</div>

[3Blue1Brown — Reinventing Entropy | Compression is Intelligence Part 1](https://youtu.be/l6DKRf-fAAM?si=e0CzpHt6EX0ZknoB)
...
[3Blue1Brown — But what is cross-entropy? | Compression is Intelligence Part 2](https://youtu.be/GlYgs6v2YfU?si=UDKTMTXXctsv10q4)



#### 1.2.5 Evaluation with Perplexity, and Bias-Variance Tradeoff

### 1.3 Parameterizing Classifcation with Logistic Regression
<small>[$\hookleftarrow$ Table of Contents](#table-of-contents)</small>

> In which we parameterize classification with a set of weights and biases known as the logistic regression model, which corresponding to a single neuron in a deep network; use the cross entropy loss; iteratively optimize such loss by straddling both linear algebra and differential calculus with iterative reweighted least squares; motivate cross entropy loss and least squares with them more general assumptions from the probabilistic and decision theoretic perspectives; and finally, evaluate
...
- [1.3 Parameterizing Classification]()
  ...
  - [1.3.3 Optimization with Iterative Reweighted Least Squares]()
  - [1.3.3 Optimization with Stochastic Gradient Descent]()
...
With a better understanding of the machine learning process that models language as a probabilistic phenomena and estimates some classifying function $\hat{f}: \mathscr{X} \to \mathscr{Y}$ where $\mathscr{X} := V$ and $\mathscr{Y} := V^{*}$ from self-supervised data $d:=\{(\mathbf{x}^{(i)}, y^{(i)})\}_{i=1}^{n}$, let us now introduce a different way of estimating $\hat{f}$ which will start our journey in earnest towards training deep neural nets in [Part II. Deep Neural Networks](./2.md). That different way is **parameter estimation**<span class="defnote">**parameter estimation**</span>.
...
with $f(v) := I^{-1} \circ \text{rng} \circ P \circ I(v)$, where the hashmap $I: V \to \N$ mapped words to indices, the stochastic matrix $P: \N \times \N \to \reals$
curried indices to their row distributions over next tokens, (todo) $\text{rng}$ mapped distributions to one specific word class index, and the inverted hash map $I^{-1}: \N \to V$ mapped the index back to the word.
The core machinery of $\hat{f}$ was the row stochastic matrix $P \in \reals^{|V| \times |V|}$ which corresponded to an `np.ndarray` with `.shape` of `(V, V)`. This stochastic matrix is displayed on the left hand side of Figure 1.3.1.
...
So, starting from the string `"to"`, we continue to map the string to it's index $i$ in the vocabulary $V$ with some hash map $I: V \to \N$, but then we need to map the index $i \in \N$ to some vector $\mathbf{x} \in \reals^{|V|}$.
...
#### 1.3.2 Optimizing Loss with Gradient Descent
<!--
- TODO: use video 2 for cross entropy loss in chapter 1.3 (pre-training with next token prediction is compression)
...
<div class="full-bleed">
<iframe height="500px" loading="lazy" src="https://www.youtube.com/embed/NrO20Jb-hy0?si=2hCmHIIIjhLq2OGf"  title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
</div>

[Welsch Labs — The Misconception that Almost Stopped AI](https://youtu.be/NrO20Jb-hy0?si=fls0s9TQicr0BG7C)
...
- [2. The Massively Parallel Multidimensional Tensor](#2-from-ipls-array-to-apls-multidimensional-array)
  - [2.1 The Three Language Problem]()
  - [2.2 Virtualizing Shapes with Strides]()
  - [2.3 Accelerating Matrix Multiplication on CPUs]() <!-- with computation and communication of rooflines -->
