...
> In which we parameterize classification with a set of weights and biases known as the logistic regression model (corresponding to a single layer of a deep net); use the cross entropy loss; iteratively optimize such loss by straddling both linear algebra and differential calculus with iterative reweighted least squares; motivate cross entropy loss and least squares with them more general assumptions from the probabilistic and decision theoretic perspectives; and finally, evaluate
...
With a better understanding of the machine learning activity that models language as a probabilistic phenomena and estimates some classifying function $\hat{f}: \mathscr{X} \to \mathscr{Y}$ where $\mathscr{X} := V$ and $\mathscr{Y} := V^{*}$ from self-supervised data $d:=\{(\mathbf{x}^{(i)}, y^{(i)})\}_{i=1}^{n}$, let us now introduce a different way of estimating $\hat{f}$.
...
The core machinery of $\hat{f}$ is the row stochastic matrix $P \in \reals^{|V| \times |V|}$ which corresponds to an `np.ndarray` with `.shape` of `(V, V)`, displayed on the left hand side of Figure 1.3.1.

In this chapter however, we will estimate $\hat{f}: V \to V$ by **parameterizing**<span class="defnote">**parameters**</span>
probabilities $p(w|h)$ rather than counting them, namely with the **weights**<span class="defnote">**weights**</span> $w_i$ and **biases**<span class="defnote">**bias**</span> $b$ of a **logistic regression**<span class="defnote">**logistic regression**</span> model, which can also be viewed as a single **neuron**<span class="defnote">**neuron**</span> from a deep net. A circuit for the logistic regression model is shown on the right hand side of Figure 1.3.1. Rather than produce the <span class="okabe-orange">orange output $p(w|h)$</span> by counting, the logistic regression model produces it by passing the <span class="okabe-blue">blue input $w$</span> into <span class="okabe-green">green sum $\sum$ and squash $\sigma$ functions</span> that are configured with <span class="okabe-purple">purple parameters $w$ and $b$</span>.
